  ...
  return (
    <PageLayout>
          ...
          {!!article.fields.Image?.[0] ? (
            <>
              <div className="flex flex-col justify-center space-y-4">
                <h1 className="font-identity text-5xl md:text-6xl">
              ...
              <div className="rounded-lg shadow-white overflow-hidden">
                <Image
                  layout="responsive"
                  src={
                    article.fields.Image[0].thumbnails.full?.url ||
                    article.fields.Image[0].url
                  }
            ...
            <>
              <div className="flex flex-col justify-center space-y-4 pb-4">
                <h1 className="font-identity text-5xl md:text-6xl mb-4">
                  {article.fields.Title}
                </h1>
                {article.fields.ByLine && (
                  <p className="text-xl font-serif">{article.fields.ByLine}</p>
      ...
      {moreArticles.length != 0 ? (
        <aside className="content-wrapper mx-auto space-y-4 border-t border-white pt-5 pb-5">
...
export const getStaticPaths: GetStaticPaths = async (context) => {
  const links = (await getBlogPosts()).filter(
    (page) => typeof page.fields.Slug === "string"
  );
  return {
