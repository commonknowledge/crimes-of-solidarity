...
} from "./SolidarityActions";
import { projectStrings } from "../data/site";
import { groupUrl } from "../data/organisingGroup";

export function useSelectedOrganisingGroup(
  groups: OrganisingGroup[],
  key = "dialogOrganisingGroupId"
) {
  const router = useRouter();
  const dialogOrganisingGroupId = router.query[key];
  const selectedOrganisingGroup = groups.find(
    (a) => a.id === dialogOrganisingGroupId
  );
...
export const OrganisingGroupCard = ({
  data,
  withPadding = true,
  withContext = true,
}: {
        ...
        <div className={cx(withPadding && "md:px-8", "p-4 bg-white")}>
          <div className="text-sm">
            <div className="flex flex-wrap tracking-tight">
              <span className="pr-3">
                {data.fields.IsUnion ? "Union" : "Organising group"}
              </span>
            ...
            <div className="pb-4" />
            <h3
              className={cx("text-3xl leading-tight font-semibold max-w-3xl")}
            >
              {data.fields.Name}
            </h3>
            {data.fields["Full Name"] && (
              <div className={"w-full pt-4 text-lg font-light space-y-2"}>
                ...
                <p>
                  ...
                  {pluralize(
                    "case",
                    data.fields["Solidarity Actions"]?.length,
                    true
                  )}{" "}
              ...
              {data.fields.Twitter && (
                <a href={data.fields.Twitter} className="block my-1">
                  <svg
                    ...
                    height="20"
                    fill="currentColor"
                    className="inline-block text-[#1da1f2]"
                  >
                  ...
                  <span className="align-baseline underline text-inherit">
                    @
                    {new URL(data.fields.Twitter).pathname.replace(/\//gim, "")}
                  </span>
                </a>
              )}
            </div>
        ...
        {withContext && (
          <div className="grid gap-[2px] grid-cols-2">
            ...
            {data.fields.countryCode?.map((code) => (
              <div
                className={cx(withPadding && "md:px-8", "p-4 bg-white")}
                key={code}
              >
